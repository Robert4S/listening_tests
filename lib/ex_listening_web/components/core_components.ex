defmodule ExListeningWeb.CoreComponents do
  @moduledoc """
    ...
    <div>
      ...
      <input
        ...
        class={[
          ...
          @errors == [] && "border-zinc-300 focus:border-zinc-400 h-full py-2 px-2",
          @errors != [] && "border-rose-400 focus:border-rose-400 h-full py-2 px-2"
        ]}
        {@rest}
      />
      <.error :for={msg <- @errors}><%= msg %></.error>
  ...
  def label(assigns) do
    ~H"""
    <label for={@for} class="block text-sm font-semibold leading-6 text-zinc-800 items-center">
      <%= render_slot(@inner_block) %>
    </label>
    """
  end
  ...
  def translate_error({msg, opts}) do
    ...
    if count = opts[:count] do
      Gettext.dngettext(ExListeningWeb.Gettext, "errors", msg, msg, count, opts)
    else
      Gettext.dgettext(ExListeningWeb.Gettext, "errors", msg, opts)
    end
  end
  ...
  slot :label, required: true, doc: "the slot for the label content"

  def file_input(%{upload: upload} = assigns) do
    ...
    />
    <.label for={@upload.ref} class={"#{@class}"}>
      <%= render_slot(@label) %>
    </.label>
    """
  end
  ...
  def dropdown(assigns) do
    ~H"""
    <div class="relative inline-block text-left" data-component="dropdown">
      <button
        type="button"
        class="hover:text-zinc-700 inline-flex w-full rounded-md bg-blue-100 py-2 px-4 h-full font-semibold shadow items-center"
        id={"#{@id}-button"}
        aria-expanded="false"
