defmodule ExListeningWeb.TeacherAuth do
  ...
  @max_age 60 * 60 * 24 * 60
  @remember_me_cookie "_ex_listening_web_teacher_remember_me"
  ...
  def log_out_teacher(conn) do
    ...
    if live_socket_id = get_session(conn, :live_socket_id) do
      ExListeningWeb.Endpoint.broadcast(live_socket_id, "disconnect", %{})
    end
  ...
  the current_teacher:
      ...
      defmodule ExListeningWeb.PageLive do
        use ExListeningWeb, :live_view

        on_mount {ExListeningWeb.TeacherAuth, :mount_current_teacher}
        ...
      end

  Or use the `live_session` of your router to invoke the on_mount callback:

      live_session :authenticated, on_mount: [{ExListeningWeb.TeacherAuth, :ensure_authenticated}] do
  ...
  def on_mount(:ensure_authenticated, _params, session, socket) do
    ...
    if socket.assigns.current_teacher do
      ...
      socket =
        socket
        |> Phoenix.LiveView.put_flash(:error, "You must be a techer to access this page")
        |> Phoenix.LiveView.redirect(to: ~p"/join")
