...
COPY --from=builder --chown=nobody:root /app/_build/${MIX_ENV}/rel/ex_listening ./

CMD mkdir /app/database
...
# ENTRYPOINT ["/tini", "--"]
RUN sqlite3 /app/exlistening.sqlite3
ENV SECRET_KEY_BASE="AEdogYWlsXxJH8vp+VyLg89sxGSrBKV7z3ffPwCQVce26xKhXMufyrV4PUdSppfa"
ENV DATABASE_PATH="/app/database/exlistening.sqlite3"
RUN /app/bin/migrate
EXPOSE 4000
CMD ["/app/bin/server"]
